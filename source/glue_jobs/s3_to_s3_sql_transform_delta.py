...
class S3ToS3SQLTransformDelta(GlueJob, S3Client):
    def __init__(self):
        super().__init__(self.init_params, self.run)
        self.spark_client = SparkClient(self.context, self.logger)

    def init_params(self):
        self.logger.info(f"[DEBUG] job_execution_params before validation: {self.config.get('job_execution_params')}")
        
        # Print Spark configuration
        print("Spark Configuration:")
        for item in self.context.spark_session.sparkContext.getConf().getAll():
            print(f"{item[0]} = {item[1]}")

        self.s3_client_boto = boto3.client("s3")
    ...
    def check_table_exists(self, database, table):
        table_names = [tbl.name for tbl in self.context.spark_session.catalog.listTables(database)]
        return table in table_names

    def create_glue_table(self, table, database, location, exists):
        df = self.rename_columns(self.context.spark_session.read.parquet(location))
    ...
    def create_iceberg_table(self, df):
        ...
        # Write the cleaned DataFrame to Iceberg using DataFrameWriterV2
    ...
    def deltaTransformation(self):
        if self.file_type == "parquet_to_iceberg":
            ...
            sourceDDF = self.context.create_dynamic_frame.from_catalog(
            ...
            sourceDDLChangesDF = self.rename_columns(self.context.spark_session.sql(data_type_changes_sql))
            ...
            # Archiving source parquet files from zone=staging to zone=processed
            self.archive_files()

        elif self.file_type == "iceberg_to_iceberg":
            self.logger.info(f"{self.log_tag} Processing Iceberg to Iceberg")
            sourceDF = self.context.create_data_frame.from_catalog(
                database=self.source_catalog_database,
                table_name=self.source_table,
                transformation_ctx=f"{self.source_table}DF",
            )
    ...
    def archive_files(self):
        ...
        paginator = self.s3_client_boto.get_paginator("list_objects_v2")
        for page in paginator.paginate(Bucket=bucket_name, Prefix=self.split_s3_uri(staging_prefix)[1]):
            for obj in page.get("Contents", []):
                ...
                try:
                    # Copy the object to the processed prefix
                    self.s3_client_boto.copy_object(
                        CopySource={"Bucket": bucket_name, "Key": source_key},
                        Bucket=bucket_name,
                        Key=target_key
                    )
                    # Delete the original file from staging
                    self.s3_client_boto.delete_object(Bucket=bucket_name, Key=source_key)
    ...
    def update_existing_table(self, target_tbl, source_tbl):
        ...
        df_target = self.context.spark_session.table(target_tbl)
        df_source = self.context.spark_session.table(source_tbl)
        ...
        for col_name in new_columns:
            ...
            self.context.spark_session.sql(alter_statement)
        ...
        # Determine the common columns that exist in both source and target
        ...
        self.context.spark_session.sql(merge_statement.strip())
    ...
    def truncate_and_load(self, target_tbl, source_tbl):
        ...
        self.context.spark_session.sql(truncate_statement)
        ...
        self.context.spark_session.sql(insert_statement.strip())

    def get_sql_queries(self):
        self.logger.info("GETTING SQL QUERIES FROM S3")
        response = self.s3_client_boto.get_object(
    ...
    def run(self):
        try:
            # Ensure global parameters and clients are initialized before running pipeline
